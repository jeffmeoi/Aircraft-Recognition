...
import utils
import json
import keypoints_detect_network_diy as keypoints_detect_network
import resnet_recognize_aircraft_diy1 as resnet_recognize_aircraft
...
def eval():
    ...
    for i, (img, anno) in enumerate(test_data):
        ...
        gray_img = np.transpose(np.array([gray_img]), (1, 2, 0))
        start_time = time.time()
        pred, prob = resnet_recognize_aircraft.predict(gray_img)
        end_time = time.time()
    ...
    print('avg time', tot_time / len(test_data))
