...
img_size = 224
n_labels = 20
model_file = '19_resnet_recognize_aircraft.pth'
...
class TypeDataset(Dataset):
    def __init__(self, annotations_file=utils.annotations_dir + 'regular.json', 
        ...
        for anno in annotations:
            ...
            seq = anno['seq']
            self.dict.append((torch.tensor(np.array([img, img, img])).float(), seq))
            flip_img = cv2.flip(img, 1)
            self.dict.append((torch.tensor(np.array([flip_img, flip_img, flip_img])).float(), seq))
...
train_data = TypeDataset(annotations_file=utils.annotations_dir + 'regular_train_set_19.json')
test_data = TypeDataset(annotations_file=utils.annotations_dir + 'regular_test_set_19.json')
